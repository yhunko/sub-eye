  ...
  static async suggestCategories(
    ...
    const [aiContext, subscriptions, categories, preferences] =
      await Promise.all([
        ...
        deps.subscriptionRepository.findByUserId(db, userId),
        deps.categoryRepository.findByUserId(db, userId),
  ...
  static async applyCategories(
    ...
    for (const suggestion of suggestions) {
      ...
      if (categoryId) {
        ...
        const created = await deps.categoryRepository.create(db, {
          userId,
      ...
      for (const subscriptionId of uniqueSubscriptionIds) {
        ...
        await deps.subscriptionRepository.update(db, subscriptionId, {
          categoryId,
        });
  ...
  static async applyOptimization(
    ...
    await runInTransaction(async (tx) => {
      for (const subscription of subscriptions) {
        ...
        await deps.subscriptionRepository.update(
