...
describe("CategoryService.deleteCategories", () => {
  it("deletes all requested user-owned categories and deduplicates ids", async () => {
    const findByIdsForUser = mock(async (_db, userId: string, ids: string[]) =>
      ids.map((id) => ({
        id,
        userId,
      })),
    ...
    const deleteByIdsForUser = mock(
      async (_db, _userId: string, ids: string[]) => ids.length,
    );
    ...
    expect(findByIdsForUser.mock.calls[0]?.[2]).toEqual(["cat_1", "cat_2"]);
    expect(deleteByIdsForUser).toHaveBeenCalledTimes(1);
    expect(deleteByIdsForUser.mock.calls[0]?.[2]).toEqual(["cat_1", "cat_2"]);
  });
  ...
  it("throws when delete result count differs from validated ids", async () => {
    await expect(
      CategoryService.deleteCategories(["cat_1", "cat_2"], "user_1", {
        repository: {
          findByIdsForUser: async (_db, userId: string, ids: string[]) =>
