...
export class ComparatorService {
  ...
  static async getQuota(
    ...
    const usage = await deps.repository.findByUserAndPeriod({
      userId,
  ...
  static async compare(
    ...
    if (context.planId === "free") {
      const consumed = await deps.repository.consumeMonthlyQuota({
        userId,
        periodKey: quotaWindow.periodKey,
      ...
      const updated = await deps.repository.incrementMonthlyQuota({
        userId,
        periodKey: quotaWindow.periodKey,
      });
