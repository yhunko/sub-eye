import type { ComparatorAiQuotaDto, PlanId } from "@subeye/shared";
import {
...
export class AiUsageService {
  ...
  static async consume(
    ...
    const consumed = await deps.comparatorRepository.consumeAiMonthlyQuota({
      userId,
    ...
    const latestUsage =
      await deps.comparatorRepository.findAiUsageByUserAndPeriod({
        userId,
        periodKey: context.quotaWindow.periodKey,
      });
