...
- **Sheets are native `formSheet` routes**, the only sheet mechanism in the app: Manage-pricing, Pause, the list filters, and the category editor. `presentation: "formSheet"` with `sheetGrabberVisible: true`, plus a fixed detent for anything holding a ScrollView (a `flex: 1` scroller has no intrinsic height, so `fitToContents` can measure to nothing). There is **no NiceModal / modal-manager equivalent** — the navigator owns presentation.
...
- **`headerSearchBarOptions` works on a pushed screen inside that nested stack** — the category picker uses the real `UISearchBar` (`placement: "stacked"`, `hideWhenScrolling: false`), glass header and all. If it ever appears not to render, suspect a stale Fast Refresh before concluding the platform cannot do it: a full relaunch was the difference here, and a hand-rolled `TextInput` lookalike was very nearly shipped over it.
