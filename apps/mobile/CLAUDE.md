...
- `_layout.tsx` also does a bare side-effect import `import "@/shared/lib/online"` (wires connectivity once). **It has no binding — do not let an auto-import cleanup delete it**, or online/offline detection dies app-wide.
...
- The server sets `.basePath("/api")` and has **no version segment**. Base URL = `${EXPO_PUBLIC_API_URL}/api`.
